...
export default function ReviewForm({ productId, productName }: Props) {
  ...
  const renderStars = (r: number) => (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(s => (
        <svg key={s} className={`w-4 h-4 ${s <= r ? "text-amber-400 fill-current" : "text-zinc-200 fill-current"}`} viewBox="0 0 20 20">
  ...
  return (
    <div className="mt-8 border-t border-zinc-100 pt-6">
      <h3 className="text-lg font-bold text-zinc-900 mb-4">Customer Reviews</h3>
      ...
      {reviews.length > 0 ? (
            ...
            <div key={r.id} className="p-4 bg-zinc-50 rounded-2xl">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-zinc-900 text-sm">{r.author}</span>
                  <span className="text-xs text-zinc-400">{new Date(r.date).toLocaleDateString()}</span>
                </div>
                {renderStars(r.rating)}
              </div>
              <h4 className="font-semibold text-zinc-800 text-sm mb-1">{r.title}</h4>
              <p className="text-sm text-zinc-600 leading-relaxed">{r.body}</p>
              {r.reply && (
                <div className="mt-3 p-3 bg-zinc-100/80 rounded-xl border-l-2 border-rose-400">
                  <p className="text-xs font-semibold text-zinc-800 mb-1">Store Response:</p>
                  <p className="text-sm text-zinc-600">{r.reply}</p>
                </div>
              )}
            </div>
          ))}
        ...
        <p className="text-sm text-zinc-500 mb-6">No reviews yet. Be the first to review this product!</p>
      )}

      {!open && !submitted && (
        <button
          onClick={() => setOpen(true)}
          className="inline-flex items-center gap-1.5 px-4 py-2 bg-white border border-zinc-200 hover:border-rose-300 text-rose-500 text-sm font-semibold rounded-xl transition-colors"
        >
